...
const Analytics = () => {
  ...
  return (
    <div>
      ...
      <h2 className="text-2xl font-semibold my-5 ml-5">
        {pathname === "/seller/profile" ? "Uploaded" : "Purchased"} This Year
      </h2>

      <div
        className="w-[83vw] sm:w-[80vw] ml-8 p-2 bg-white rounded-2xl shadow-md flex
      ...
      {/* My three expense caqrd will be here  */}
        {
          !thisMonth?.length ? (
            ...
            <div className="flex flex-col sm:flex-row justify-between gap-2 mb-10">
              <ExpenseCard data={thisWeek} title={`${pathname == "/seller/profile" ? "Earned" : "Spent"} This Week`} dataKey="price"  
              value={pathname == "/seller/profile" ? calculateTotalForSeller(thisWeek) : calculateTotalForBuyer(thisWeek)}/>
              <ExpenseCard data={thisMonth} title={`${pathname == "/seller/profile" ? "Earned" : "Spent"} This Month`} dataKey="price" 
              value={pathname == "/seller/profile" ? calculateTotalForSeller(thisMonth) : calculateTotalForBuyer(thisMonth)}/>
              <ExpenseCard data={tillNow} title={`${pathname == "/seller/profile" ? "Earned" : "Spent"} Till Now`} dataKey="price" 
