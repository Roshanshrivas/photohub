...
const Navbar = () => {
  ...
  const refreshToken = async () => {
    try {
      ...
      const message = error?.response?.data?.message || error?.message || "Error from the server";
      toast.error(message);
      dispatch(logout());
    }
  };
  ...
  return (
    <nav
      className={`flex flex-col sm:flex-row justify-between items-start sm:items-center px-5 py-4 
      ${
    ...
    >
      ...
      <div className="flex justify-between items-center">
        ...
        <Link to="/" className="font-bold text-3xl">
           <img src="/photohubLog.jpg" alt="logo" className="w-[160px] ml-5" />
        </Link>
      </div>

      {/* list of other tabs */}
      <ul className="flex gap-5 text-lg font-semibold text-gray-700 ml-5 sm:ml-0">
